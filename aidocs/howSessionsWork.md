...
1. **A bigger ceiling** (`src/index.ts`, commit cdaa5eb) — re-execs with `--max-old-space-size=8192` (overridable via `XCTRACE_QUERY_MCP_MAX_HEAP_MB`) if the launch config didn't already request one. A mitigation, not a fix — an even larger table can still exceed it.
